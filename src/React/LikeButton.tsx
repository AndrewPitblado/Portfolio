  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(likeDocRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        console.warn("Likes document not found at likes/counter.");
      }
    });
  ...
  return (
    <div className="flex items-center">
      <button
        ...
        className={`group relative flex h-10 w-40 items-center justify-center rounded-full border-2 p-3 transition-all duration-300 ease-in-out ${borderColorClass} ${!isLiked ? "md:hover:border-[var(--white)]" : ""} ${triggerAnimation ? "animate-scale" : ""} `}
      >
        <svg
        ...
        <span
          className={`pl-3 text-sm transition-all duration-300 ease-in-out ${animateLikes ? "animate-scale" : ""} text-[var(--white)]`}
        >
          {likes} Likes
        </span>
      </button>
